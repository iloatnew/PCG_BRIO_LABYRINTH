...
///</summary>
///************************************************************
public class MazeLoaderCurriculum : MonoBehaviour
{
	...
	void Awake()
	{
		...
		var halfLengthRow = (2 - 1) * GetSize() / 2;
		var halfLengthColumn = (2 - 1) * GetSize() / 2;
		shifting = new Vector3(-halfLengthRow, 0, -halfLengthColumn);
	...
	{
		player = Instantiate(ball, transform.TransformPoint(new Vector3(0 * size, -(size / 2f) + 1f, 0 * size) + shifting), Quaternion.identity, maze.transform);
		player.name = "Player";
