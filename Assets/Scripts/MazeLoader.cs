...
public class MazeLoader : MonoBehaviour {
		...
		for (int r = 0; r < mazeRows; r++){
			for (int c = 0; c < mazeColumns; c++){
				...
				if (trapPositions.Contains(curPosition)){
					...
					mazeCells[r, c].floor = Instantiate(trap, new Vector3(r * size, -(size / 2f), c * size), Quaternion.identity) as GameObject;
	...
	{
		player = Instantiate(ball, new Vector3(0 * size, -(size / 2f) + 5f , 0 * size), Quaternion.identity);
		player.name = "Player";
